...
Type objective_function<Type>::operator() ()
{
  ...
  for(int i=0;i<obs_w.rows();i++){
    ...
    if (dist_wobs==1){
      ...
      ans_w+=log(obs_w(i,2))-dnorm(log(obs_w(i,2)),logwaa(a,y)-Type(0.5)*shape*shape,shape,true); //using lognormal distribution with bias correction
    }
  }
  ...
  maa_diff.fill(-1);
  Type multi_g;
  array<Type> N_mat(alpha_g.size(),maa.cols());
  N_mat.fill(0.0);
  
  for(int j=0;j<maa.cols();j++){ //最初の年は除く（2年目から）
    for(int i=0;i<maa.rows();i++){
      if(g_fix(i)<0.0){
